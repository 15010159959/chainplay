...
var dappAddress = "n1gVNLmP2B2FGfiwqy1f2nx1xHLqSSwV3iR";
var nebulasUrl = "https://chainplay.io";
var payCallbackUrl = NebPay.config.mainnetUrl;


if (env == 'testnet'){
    dappAddress = 'n1nhDv68upBQRu5L4prDxi8DxNsR2ZcQ1hq';
...
class Melo {
    ...
    getInfo() {
        ...
        neb.api.call(account, dappAddress, value, nonce, gas_price, gas_limit, contract).then(function (resp) {
            ...
            $("#total").text(( (result.eggReward+result.inviteReward+result.bonus ) / 1000000000000000000).toFixed(4) + " NAS")
            // " + result.eggCount + "次");
                ...
                if (respObject.code === 0 && respObject.data.status !== 2) {
                    ...
                    if (respObject.data.status === 1) {
                        ...
                        if (callFunction == "buyKey") {
                            ...
                            if (execute_result.reward){
                                msg = "<p>哇塞！你不仅购买成功了，你还中奖了</p>";
                                msg += "恭喜你,你将获得"+ execute_result.reward / 1000000000000000000 + " nas,别忘记了领奖和分享给朋友哦"
                                msg += "你购买了 " + execute_result.amount + " 个珍珠, 每个价格为" + execute_result.price / 1000000000000000000 + " nas";
                            }
