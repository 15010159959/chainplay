...
const nebulasUrl = "https://chainplay.io";
...
const payCallbackUrl = NebPay.config.mainnetUrl;


var intervalQuery ;
class Melo {
    constructor(dappAddress, language) {
    ...
    getInfo() {
        ...
        var that = this
        neb.api.call(this.account ? this.account : defaultAccount, dappAddress, value, nonce, gas_price, gas_limit, contract).then(function (resp) {
    ...
    nebpayListener(serialNumber, result) {
        ...
        if (respString.indexOf("rejected") >0) {
            ...
            console.log("========response of push: " + JSON.stringify(result) + " respString:"+respString)
            hideLoad()
...
var melo = new Melo(dappAddress, language);
// 页面加载时请求数据
$(function () {  
    // $("#search_value").val() 搜索框内的值
    $(".modal.loading").modal("show");

    showDialog(tip, '转化兽下一轮敬请期待，我们正在全力的优化')
